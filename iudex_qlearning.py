...
if __name__ == "__main__":
    ...
    fill_buffer(buffer, env, buffer_size)  # TODO: Load if possible
    ...
    for i in range(n_episodes):
        ...
        state = gamestate2np(env.reset())
        ep_reward = 0.
        t = 0
        eps = eps_scheduler.epsilon
        eps_scheduler.step()
        while not done:
            action = env.action_space.sample() if np.random.rand() < eps else agent(state)
            next_state, reward, done, _ = env.step(action)
            next_state = gamestate2np(next_state)
